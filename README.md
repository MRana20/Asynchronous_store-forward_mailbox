...
Each user generates a long-term RSA keypair (their identity). They also generate a DH keypair (an ephemeral "prekey") and sign the prekey's public value with their RSA private key - signing a SHA256 hash of it, since that's how RSA signatures actually work. They publish their RSA public key + signed prekey to a shared "directory" (just a file or tiny local server).
